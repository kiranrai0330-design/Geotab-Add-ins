geotab.addin.descartesDashboard = () => {
    ...
    return {
        ...
        // FIX: Added focus() lifecycle method — called when user navigates back to the add-in
        focus(freshApi, state) {
            api = freshApi;
            this.update();
        },

        // FIX: Added blur() lifecycle method — called when user navigates away
        blur() { },
        ...
        async update() {
            ...
            document.getElementById('reportContent').innerHTML =
                '<tr><td colspan="100" style="text-align:center;padding:40px;color:#999;">Loading data...</td></tr>';
            document.getElementById('kpi-missing').innerText = '—';
            document.getElementById('kpi-partial').innerText = '—';
            document.getElementById('kpi-compliant').innerText = '—';
            document.getElementById('kpi-score').innerText = '—';

            // Build date range for column headers
            masterData = {};
            currentDates = [];
            ...
            try {
                const fromDate = new Date(startVal + 'T00:00:00').toISOString();
                // FIX: Use end-of-day so the last selected date is fully included
                const toDate = new Date(endVal + 'T23:59:59').toISOString();
                ...
                // Process Trips into Master Data
                trips.forEach(t => {
                    ...
                    if (!masterData[dId]) {
                        ...
                        if (!fullName) fullName = u.name || dId;
                    ...
                    // FIX: Convert meters to miles (Geotab returns distance in meters)
                    masterData[dId].days[date].miles += t.distance / 1609.34;
                    masterData[dId].days[date].vehicles.add(deviceMap[t.device.id] || t.device.id);
                });

                // Match DVIRs to driver and day
                dvirs.forEach(log => {
                    // FIX: Guard against DVIRs with no driver assigned
                    if (!log.driver || !log.driver.id) return;

                    const dId = log.driver.id;
                    const date = log.dateTime.split('T')[0];

                    // FIX: Create a driver entry even if they had no trips that day,
                    // so inspections aren't silently dropped
                    if (!masterData[dId]) {
                        const u = userMap[dId] || {};
                        let firstName = u.firstName || "";
                        let lastName = u.lastName || "";
                ...
                document.getElementById('reportContent').innerHTML =
                    '<tr><td colspan="100" style="text-align:center;padding:40px;color:#cc0000;">Failed to load data. Please try again.</td></tr>';
            }
        },
